...
def update_images_from_stock(tree_old: ET.ElementTree, tree_new: ET.ElementTree):
...
def map_attach_properties(tree_old: ET.ElementTree, tree_new: ET.ElementTree):
...
def modify_labels(tree_old: ET.ElementTree, tree_new: ET.ElementTree):
    pass  # existing logic here
...
def map_property_names(tree_old: ET.ElementTree, tree_new: ET.ElementTree):
...
def handle_deprecated_properties(tree_old: ET.ElementTree, tree_new: ET.ElementTree):
    deprecated_mappings = {
        'rules-hint': None,
        'stock': 'icon-name',
        'shadow-type': None,
...
def map_shadow_type(tree_old: ET.ElementTree, tree_new: ET.ElementTree):
...
def ensure_button_labels(tree_old: ET.ElementTree, tree_new: ET.ElementTree):
...
def output_shadow_types():
    with open("widget_shadow_types.py", "w", encoding="utf-8") as file:
...
modification_functions = [
    modify_labels,
    update_images_from_stock,
    map_attach_properties,
    map_property_names,
    handle_deprecated_properties,
    ensure_button_labels,
    map_shadow_type
]

base_old = Path("/home/chris/repositories/ghini.desktop.new/bauble")
base_new = Path("/home/chris/debug/ghini.desktop/bauble")

parser = ET.XMLParser(remove_blank_text=True)

for old_file in base_old.rglob("*.glade"):
    rel_path = old_file.relative_to(base_old)
    new_file = base_new / rel_path
    ...
    if new_file.exists():
        ...
        xml_content = ET.tostring(tree_new, pretty_print=True, xml_declaration=True, encoding="UTF-8").decode("UTF-8")
