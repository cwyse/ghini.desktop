...
plugins = setuptools.find_packages(
    where="bauble/plugins", exclude=["test", "bauble.*.test", "ghini.*.test"]
)
        ...
        for p in patterns:
            ...
            if matches != []:
                ...
                if index != -1:
                    ...
                    install_dir = pkg_dir
                data_files.append(
                    (install_dir, [m.replace(os.sep, "/") for m in matches])
                )
    ...
    class NsisCmd(Command):
        ...
        def finalize_options(self):
            ...
            if not os.path.exists(self.nsis_script):
                raise Exception(
                    "can not find nsis build script at: %s" % self.nsis_script
                )
...
class build(_build):
    def run(self):
        ...
        if not spawn.find_executable("msgfmt"):
            msg = (
                "** Error: Building Ghini requires the gettext utilities "
...
class install(_install):
    ...
    def run(self):
        ...
        if not self.single_version_externally_managed:
            ...
            old_egg_dirs = [
                a
                for (a, b, c) in os.walk(self.install_data)
                if (
...
class docs(Command):
    ...
    def run(self):
        try:
            ...
            print(
                "Building the docs requires the "
...
setuptools.setup(
    ...
    long_description_content_type=project_info.get("readme", {}).get(
        "content-type", ""
    ),
