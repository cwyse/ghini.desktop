...
from sqlalchemy.orm import relation, backref, validates, synonym, Session
...
bauble.db.Base.metadata.create_all(bind=bauble.db.engine)
...
def add_genera_callback(families):
    session = bauble.db.session()  # Use bauble.db.session()
    family = session.merge(families[0])  # Use Session.merge()
...
def remove_callback(families):
    ...
    session = Session.object_session(family)  # Use Session.object_session()
    ...
    try:
        ...
        session.delete(obj)
        session.commit()
    except DBAPIError as e:  # Use DBAPIError instead of Exception
...
FamilyNote = bauble.db.make_note_class(
    "Family",
    compute_serializable_fields,
    cols={"family": Column(Integer, ForeignKey("family.id"), nullable=True)},
)
...
class FamilySynonym(db.Base):
    ...
    # columns
    family_id = Column(
        Integer, ForeignKey("family.id", ondelete="cascade"), nullable=False
    )
...
Family.genera = relationship(
    "Genus",
    order_by=[Genus.genus],
    backref=backref("family", cascade="all, delete-orphan"),
)
...
class FamilyEditorPresenter(editor.GenericEditorPresenter):
    """
    Presenter for the Family editor.
    """

    widget_to_field_map = {
        "fam_family_entry": "epithet",
    ...
    def connect_signals(self):
        for widget, field in list(self.widget_to_field_map.items()):
            self.view.connect_widget(widget, self.set_model_attr, field)

        self.notes_presenter.connect_signals()

        self.synonyms_presenter.connect_signals()
    ...
    def set_model_attr(self, field, value):
...
class SynonymsPresenter(editor.GenericEditorPresenter):
    ...
    def on_add_button_clicked(self, button, data=None):
        ...
        adds the synonym from the synonym entry to the list of synonyms for
            this species
        """
        syn = FamilySynonym(synonym=self._selected)
        self.model.synonyms.append(syn.synonym)
    ...
    def on_remove_button_clicked(self, button, data=None):
        ...
        msg = (
            ...
            "current family?\n\n<i>Note: This will not remove the family %s "
            "from the database.</i>" % (s, s)
        )
        if utils.yes_no_dialog(msg, parent=self.view.get_window()):
            # remove from treeview and model
...
class SynonymsExpander(InfoExpander):
    ...
    def __init__(self, widgets):
        ...
        self.widgets.remove(synonyms_box)
