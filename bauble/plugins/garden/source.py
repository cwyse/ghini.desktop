...
# In SQLAlchemy 1.4, ORM query functions are imported from sqlalchemy.orm
from sqlalchemy import Column, Unicode, Integer, ForeignKey, Float, UnicodeText
from sqlalchemy.exc import DBAPIError
from sqlalchemy.orm import relation, backref, selectinload

# In SQLAlchemy 1.4, create_engine and sessionmaker are imported from sqlalchemy.future
...
# In SQLAlchemy 1.4, the Base class is created using the declarative_base() function
...
collection_edit_action = view.Action(
    "collection_edit",
    _("_Edit"),
    callback=collection_edit_callback,
    accelerator="<ctrl>e",
)
collection_add_plant_action = view.Action(
    "collection_add",
    _("_Add plants"),
    callback=collection_add_plants_callback,
    accelerator="<ctrl>k",
)
collection_remove_action = view.Action(
    "collection_remove",
    _("_Delete"),
...
class Source(Base):
    ...
    # In SQLAlchemy 1.4, use selectinload for efficient loading of related objects
    source_detail = relationship(
        "Contact",
        uselist=False,
        backref=backref("sources", cascade="all, delete-orphan", viewonly=True),
        viewonly=True,
    )
    ...
    collection = relationship(
        ...
        backref=backref("source", uselist=False, viewonly=True),
        viewonly=True,
    )
    ...
    plant_propagation = relationship(
        ...
        backref=backref("used_source", uselist=True, viewonly=True),
        viewonly=True,
    )
...
class Collection(Base):
    ...
    region = relationship(GeographicArea, uselist=False, viewonly=True)
...
class CollectionPresenter(editor.ChildPresenter):
    ...
    PROBLEM_BAD_LATITUDE = str(random.random())
    PROBLEM_BAD_LONGITUDE = str(random.random())
    PROBLEM_INVALID_DATE = str(random.random())
    PROBLEM_INVALID_LOCALE = str(random.random())
    ...
    def set_model_attr(self, field, value, validator=None):
        ...
        # Check if the locale field is empty and add or remove a problem accordingly
        ...
        # Check if the field is 'longitude' or 'latitude' and update the sensitivity of widgets accordingly
        ...
        # Check if the field is 'elevation' and update the sensitivity of the altacc_entry widget
        if field == "elevation":
            sensitive = self.model.elevation is not None
            self.view.widgets.altacc_entry.set_sensitive(sensitive)

        # Refresh the sensitivity of the parent presenter
...
def _parse_lat_lon(direction, text):
    ...
    if len(parts) == 1:
        ...
        deg, min, sec = list(map(Decimal, parts))
...
source_detail_edit_action = view.Action(
    "source_detail_edit",
    _("_Edit"),
    callback=source_detail_edit_callback,
    accelerator="<ctrl>e",
)
source_detail_remove_action = view.Action(
...
class Contact(db.Model, db.Serializable, db.WithNotes):
    ...
    source_type = Column(
        types.Enum(
            ...
            name="source_type",
            values_callable=lambda obj: [i[0] for i in source_type_values],
            native_enum=False,
        ),
        default=None,
    )
